...
export function createUpdateCommand(): Command {
    return new Command('update')
        ...
        .action(async () => {
            console.log(chalk.cyan('⠋ Checking for updates to @open-agent-studio/agent...'));
            ...
            try {
                ...
                execSync('npm install -g @open-agent-studio/agent@latest', { stdio: 'inherit' });
                ...
                console.log(chalk.yellow('\nYou can try updating manually with: npm install -g @open-agent-studio/agent@latest'));
                process.exit(1);
            }
        });
}
